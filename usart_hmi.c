...
void TJCPrintf(const char *str, ...)
{
    ...
    while ((i < STR_LENGTH) && (i < strlen(buffer))) {
        HAL_UART_Transmit_IT(&LCD_huart, (uint8_t *)(buffer) + i++, 1);
        while (LCD_huart.gState != HAL_UART_STATE_READY)
            ; // 等待发送完毕
    }

    HAL_UART_Transmit_IT(&LCD_huart, &end, 1); // 这个函数改为你的单片机的串口发送单字节函数
    while (LCD_huart.gState != HAL_UART_STATE_READY);// 等待发送完毕
    HAL_UART_Transmit_IT(&LCD_huart, &end, 1); // 这个函数改为你的单片机的串口发送单字节函数
    while (LCD_huart.gState != HAL_UART_STATE_READY);// 等待发送完毕
    HAL_UART_Transmit_IT(&LCD_huart, &end, 1); // 这个函数改为你的单片机的串口发送单字节函数
    while (LCD_huart.gState != HAL_UART_STATE_READY)
...
void Init_HMI(void)
{
    ...
    HAL_UART_Receive_IT(&LCD_huart, RxBuff, 1); // 使能串口接收中断
}
